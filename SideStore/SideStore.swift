    ...
    func startRefresh() async throws {
        ...
        if (sideStorePid <= 0 || getpgid(sideStorePid) <= 0) && launchContinuation == nil {
            ...
            guard let liveProcessURL = UserDefaults.lcMainBundle().builtInPlugInsURL?.appendingPathComponent("LiveProcess.appex"),
                ...
                throw NSError(domain: "SideStore", code: 1, userInfo: [NSLocalizedDescriptionKey: "Unable to locate the required extension. Reinstall AppBox and keep all app extensions enabled."])
            }
            ...
            do {
                ...
                throw NSError(domain: "SideStore", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to start the required extension: \(error). Reinstall AppBox and keep all app extensions enabled."])
            }
            guard let ext else {
                return
            }
